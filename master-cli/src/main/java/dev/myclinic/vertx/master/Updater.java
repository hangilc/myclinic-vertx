...
abstract class Updater<M, C> {
    ...
    public abstract String getNameOfCSV(C csv);
                        ...
                        if (!isEqualMasterAndCSV(cur, csv)) {
                            if (validFrom.equals(getValidFromOfMaster(cur))) {
                                ...
                                if (validFrom.isAfter(getValidFromOfMaster(cur))) {
                                    doInvalidateAndEnter(conn, cur, csv, validFrom, isDryRun);
                                } else {
                                    throwError(String.format("Invalid validFrom %s", validFrom.toString()), cur);
                                }
                            }
                        }
                    }
    ...
    private void doInvalidateAndEnter(Connection conn, M cur, C csv, LocalDate validFrom, boolean isDryRun)
            throws Exception {
        if (isDryRun) {
            report(String.format("INVALIDATE-AND-ENTER at %s", validFrom.toString()), cur);
            reportDetail("old", cur);
            reportDetail("new", csv);
        } else {
            LocalDate prevDate = prevDay(validFrom);
            int n = updateMasterValidUpto(conn, cur, prevDate);
            if (n != 1) {
                throwError("Failed to update valid_from", cur);
            }
            insertMaster(conn, csv, validFrom);
            M m = getCurrentMaster(conn, getMasterCodeOfCSV(csv), validFrom);
            if (m == null) {
                String msg = String.format("Failed to enter master: %s: %s", csv.toString(), validFrom.toString());
                throwError(msg);
            }
            report("ENTERED", getMasterCodeOfCSV(csv), getNameOfCSV(csv));
            reportDetail("new", csv);
        }
    }
