...
public class Backend {
    ...
    public void suspendExam(int visitId) {
        VisitDTO visit = getVisit(visitId);
        if (visit != null) {
            WqueueDTO wq = getWqueue(visit.visitId);
            if( wq != null ){
                wq.waitState = WqueueWaitState.WaitReExam.getCode();
                updateWqueue(wq);
            }
        }
    }
    ...
    public List<Integer> listVisitIdInDateInRange(LocalDate fromDate, LocalDate uptoDate){
...
//            updateCharge(cur);
    ...
    public void enterPayment(PaymentDTO payment) {
